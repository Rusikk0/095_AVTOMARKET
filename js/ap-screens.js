...
(function (global) {
  ...
  async function renderStoreList() {
    ...
    try {
      ...
      if (!stores.length) {
        list.innerHTML = '<div class="empty">У вас пока нет магазинов. Создайте новый.</div>';
        return;
      }
  ...
  function bindForms() {
    ...
    if (regForm) {
      regForm.onsubmit = async function (e) {
        e.preventDefault();
        if (global.ApAccess && !global.ApAccess.allowPublicSignup()) {
          toastErr({ message: 'Регистрация отключена. Обратитесь к администратору.' });
          return;
        }
        ...
        try {
          var data = await global.ApAuth.signUp(email, pass, name);
          if (data.session) {
            toastOk('Аккаунт создан. Выберите или создайте магазин.');
            showScreen('auth-stores');
            await renderStoreList();
            return;
          }
          toastOk(
            'На почту ' + email + ' отправлено письмо. Откройте ссылку «Подтвердить» в письме — ' +
            'откроется это же окно браузера (http://localhost:5500). Затем нажмите «Войти» с тем же паролем.',
            'ok'
          );
          showScreen('auth-login');
        } catch (err) {
    ...
    document.getElementById('btn-show-register') && (document.getElementById('btn-show-register').onclick = function () {
      if (global.ApAccess && !global.ApAccess.allowPublicSignup()) {
        toastErr({ message: 'Регистрация только через администратора' });
        return;
      }
    ...
    document.getElementById('btn-goto-create-store') && (document.getElementById('btn-goto-create-store').onclick = function () {
      if (global.ApAccess && !global.ApAccess.allowPublicStoreCreate()) {
        toastErr({ message: 'Создание магазина отключено' });
        return;
      }
