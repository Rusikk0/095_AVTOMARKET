...
(function (global) {
  ...
  async function replayOp(c, storeId, op) {
    switch (op.type) {
      ...
      case 'deleteDocument':
        await c.from('document_items').delete().eq('document_id', op.payload.id);
        await c.from('documents').delete().eq('id', op.payload.id);
        break;
    }
  }
  ...
  function deferredToRow(d, sId) {
    return {
      id: d.id || undefined, store_id: sId,
      customer_name: d.customerName || '', customer_phone: d.customerPhone || '',
      product_id: d.productId || null, product_code: d.productCode || '',
      product_name: d.productName || '', quantity: d.quantity || 1,
      unit_price: d.unitPrice || 0, total: d.total || 0,
  ...
  async function loadAll() {
    ...
    cache.debtors = (q[12].data || []).map(debtorFromRow);
    var localDebts = cache.debts || [];
    var serverDebtIds = new Set((q[13].data || []).map(function (d) { return d.id; }));
    cache.debts = (q[13].data || []).map(debtFromRow);
    localDebts.forEach(function (ld) { if (!serverDebtIds.has(ld.id)) cache.debts.push(ld); });

    var localDeferred = cache.deferred || [];
    var serverDefIds = new Set((q[14].data || []).map(function (d) { return d.id; }));
    cache.deferred = (q[14].data || []).map(deferredFromRow);
    localDeferred.forEach(function (ld) { if (!serverDefIds.has(ld.id)) cache.deferred.push(ld); });

    var localDocs = cache.documents || [];
    cache.documents = (q[15].data || []).map(documentFromRow);
    var serverDocIds = new Set(cache.documents.map(function (d) { return d.id; }));
    localDocs.forEach(function (ld) { if (!serverDocIds.has(ld.id)) cache.documents.push(ld); });

    var localDocItems = cache.documentItems || [];
    cache.documentItems = (q[16].data || []).map(documentItemFromRow);
  ...
  function setDocuments(arr) {
    cache.documents = arr.slice();
    persistCacheToLocal();
    enqueue(function () { return syncDocuments(arr); });
  }

  function setDocumentItems(arr) {
    cache.documentItems = arr.slice();
    persistCacheToLocal();
    enqueue(function () { return syncDocumentItems(arr); });
  }
  ...
  async function syncDeferred(arr) {
    ...
    var id = sid();
    if (!c || !id) return;
    var rows = arr.map(function (d) { return deferredToRow(d, id); });
    if (rows.length) {
  ...
  async function syncDocuments(arr) {
    ...
    var id = sid();
    if (!c || !id) return;
    var rows = arr.map(function (d) { return documentToRow(d, id); });
    if (rows.length) {
  ...
  async function syncDocumentItems(arr) {
    ...
    var id = sid();
    if (!c || !id) return;
    var rows = arr.map(function (d) { return documentItemToRow(d, id); });
    if (rows.length) {
  ...
  async function deleteDocument(docId) {
    ...
    if (c && isOnline) {
      try {
        await c.from('document_items').delete().eq('document_id', docId);
        await c.from('documents').delete().eq('id', docId);
        return;
      } catch (err) {
