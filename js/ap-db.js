...
(function (global) {
  ...
  async function createSaleTransaction(payload) {
    ...
    var header = {
      ...
      kaspi_amount: payload.kaspiAmount || 0,
  ...
  async function createWriteOff(payload) {
    ...
    if (payload.productId) {
      ...
      if (p) {
        ...
        var up = await c.from('products').update({ quantity: newQty, updated_at: new Date().toISOString() }).eq('id', p.id);
  ...
  async function createAudit(payload) {
    ...
    for (var i = 0; i < payload.items.length; i++) {
      ...
      if (item.productId) {
        var up = await c.from('products').update({ quantity: item.qtyFact, updated_at: new Date().toISOString() }).eq('id', item.productId);
        if (up.error) throw up.error;
      }
    }
  ...
  async function createReturnTransaction(payload) {
    ...
    for (var i = 0; i < payload.items.length; i++) {
      ...
      if (it.productId) {
        ...
        if (p) {
          ...
          var pUp = await c.from('products').update({ quantity: newQty, updated_at: new Date().toISOString() }).eq('id', it.productId);
          if (pUp.error) throw pUp.error;
        }
      }

      var sItem = await c.from('sale_items').select('*').eq('id', it.saleItemId).single();
      if (!sItem.error && sItem.data) {
        var newSaleItemQty = Math.max(0, Number(sItem.data.quantity) - it.quantity);
        var newSaleItemTotal = Math.max(0, Number(sItem.data.line_total) - it.refundAmount);
        var siUp = await c.from('sale_items').update({
          quantity: newSaleItemQty,
          line_total: newSaleItemTotal
        }).eq('id', it.saleItemId);
        if (siUp.error) throw siUp.error;
      }
    }
    ...
    if (!sale.error && sale.data) {
      ...
      var sUp = await c.from('sales').update({
        total: newSaleTotal
    ...
    if (payload.customerId && payload.totalRefund > 0) {
      ...
      if (cust) {
        ...
        var bonusToDeduct = Math.round(payload.totalRefund * 0.01);
        var newBonus = Math.max(0, (Number(cust.bonusBalance) || 0) - bonusToDeduct);
        
        var cUp = await c.from('customers').update({
          spent: newSpent,
    ...
    var expRow = {
      ...
      note: 'Возврат по чеку № ' + payload.saleId.slice(-6),
      user_name: payload.userName,
      expense_date: new Date().toISOString(),
      status: 'active'
    };
    var expIns = await c.from('expenses').insert(expRow);
    if (expIns.error) throw expIns.error;
