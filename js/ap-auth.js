...
(function (global) {
  ...
  async function signUp(email, password, displayName) {
    // Block self-registration: only admin can invite users
    var allowSignup = global.ApAccess ? global.ApAccess.allowPublicSignup() : false;
    if (!allowSignup) {
      throw new Error('Самостоятельная регистрация отключена. Обратитесь к администратору магазина.');
    }
    var c = client();
  ...
  async function createStore(name) {
    // Block self-service store creation: only existing admins can create new stores
    var allowCreate = global.ApAccess ? global.ApAccess.allowPublicStoreCreate() : false;
    if (!allowCreate) {
      // Allow only if user is already admin of at least one store
      var myStores = await fetchMyStores();
      var isAdminAnywhere = myStores.some(function (s) { return s.role === 'admin'; });
      if (!isAdminAnywhere) {
        throw new Error('Создание магазина отключено. Обратитесь к администратору.');
      }
    }
    var c = client();
    if (!c) throw new Error('Supabase не настроен');
  ...
  async function inviteCashier(storeId, email, password, displayName) {
