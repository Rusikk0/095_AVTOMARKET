...
(function (global) {
  ...
  function buildInvoiceHTML(doc) {
    ...
    html += '<div style="border:1px solid #000;padding:12px;color:#000"><strong>Поставщик:</strong><br>' + escapeHtml(storeName) + '<br>БИН/ИИН: ' + escapeHtml(bin) + '</div>';
    html += '<div style="border:1px solid #000;padding:12px;color:#000"><strong>Покупатель:</strong><br>' + escapeHtml(doc.customerName || '—') + '<br>' + (doc.customerPhone ? 'Телефон: ' + escapeHtml(doc.customerPhone) : '') + '</div>';
    html += '</div>';
